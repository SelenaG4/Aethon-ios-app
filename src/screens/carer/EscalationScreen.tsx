...
export default function EscalationScreen() {
  ...
  const buildSummary = useCallback(() => {
    const subject = `Aethon: ${clientName} - physician review requested`
    const body = [
      `Carer: ${CURRENT_CARER.name}`,
      `Date: ${new Date().toLocaleString('de-CH')}`,
      `Reason: ${reason.trim()}`,
      '',
      'Recent visit notes:',
      ...recentNotes.map((n) => `${formatNoteDate(n.created_at)} - ${n.transcript ?? ''}`),
      '',
      'Generated by Aethon, a care coordination tool.',
      'This is not a clinical alerting system. Please reply to the carer.',
    ].join('\n')
    return { subject, body }
  }, [clientName, reason, recentNotes])
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        <View style={styles.headerCard}>
          ...
          <AppText style={styles.headerCardPhysician}>To {physicianName}</AppText>
        </View>
        ...
        {recentNotes.length === 0 ? (
          <AppText style={styles.emptyNote}>No recent notes</AppText>
        ) : (
          recentNotes.map((note) => (
            <View key={note.id} style={styles.noteCard}>
              ...
              <AppText style={styles.noteCardTranscript}>
                {note.transcript ? truncate(note.transcript, 120) : '(no transcript yet)'}
              </AppText>
            </View>
          ))
        )}
        ...
        <TextInput
          ...
          placeholder="What should the physician know?"
